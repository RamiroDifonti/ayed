...
sparse_vector_t::sparse_vector_t(const vector_t<double>& v, const double eps)
    : pv_(), nz_(0), n_(0) {
  n_ = v.get_size(); //n es el tamaño del vector original.
  for(int i = 0; i < n_; i++) {
    if(IsNotZero(v.at(i), eps)) {
      nz_++; //para saber el tamaño del vector disperso, se cuenta el numero de variables distintas de 0, en el original.
    }
  }
  pv_.resize(nz_); //la dimension del vector de pares es nz
  for(int i = 0, j = 0; i < n_; i++) {
    if(IsNotZero(v[i], eps)) {
      pair_t<double>w(v.at(i), i);
      pv_.set_val(j, w); //asignamos los valores y los indices
      j++;
    }
  }
}
